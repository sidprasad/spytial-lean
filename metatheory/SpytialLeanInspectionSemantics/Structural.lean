...
/-- Relations whose meanings are generated by the owned relationalizer. -/
public inductive GeneratedRelation {Ty : Type u} (L : Language.{u, v} Ty) where
  | field (field : Field L)
  | graph (operation : L.Op)

/-- Extend a vocabulary with generated constructor fields and operation graphs. -/
@[reducible, expose] public def Signature.withFields {Ty : Type u} (base : Signature Ty)
    (L : Language.{u, v} Ty) : Signature Ty where
  Relation := base.Relation ⊕ GeneratedRelation L
  columns
    | .inl relation => base.columns relation
    | .inr (.field field) => [L.result field.constructor, field.sort]
    | .inr (.graph operation) => L.params operation ++ [L.opResult operation]
...
/-- A position is preserved by a type-preserving argument map. -/
public theorem At.map {Ty : Type u} {Source : Ty → Type v} {Target : Ty → Type w}
    (transform : ∀ {sort}, Source sort → Target sort) :
    ∀ {sorts : List Ty} {arguments : Arguments Source sorts} {position : Nat}
      {sort : Ty} {entry : Source sort}, arguments.At position entry →
        (arguments.map transform).At position (transform entry)
  | _, _, _, _, _, .here => .here
  | _, _, _, _, _, .there rest => .there (At.map transform rest)

/-- An argument selected at a position occurs in the argument list's typed atom inventory. -/
public theorem At.mem_atoms {Ty : Type u} {Entry : Ty → Type v} :
    ∀ {sorts : List Ty} {arguments : Arguments Entry sorts} {position : Nat}
      {sort : Ty} {entry : Entry sort}, arguments.At position entry →
        TypedAtom.mk sort entry ∈ arguments.atoms
  | _, _, _, _, _, .here => by simp [Arguments.atoms]
  | _, _, _, _, _, .there rest => by
      simp only [Arguments.atoms, List.mem_cons]
      exact Or.inr (At.mem_atoms rest)
...
/-- The fixed meaning of an operation graph: its final column is the operation applied to all
preceding columns. -/
@[expose] public def graphHolds (sem : Semantics World L base context) (operation : L.Op) :
    Arguments sem.Carrier (L.params operation ++ [L.opResult operation]) → Prop :=
  fun point => sem.operation operation point.init = point.last

/-- The relational model over the base vocabulary and the owned generated relations. -/
@[reducible, expose] public def model (sem : Semantics World L base context) :
    Model World (base.withFields L) where
  Carrier := sem.Carrier
  holds world
    | .inl relation, args => sem.holds world relation args
    | .inr (.field field), args => sem.fieldHolds field args
    | .inr (.graph operation), args => sem.graphHolds operation args
...
/-- Mapping term denotation pointwise produces the same semantic argument list as `denoteArgs`. -/
public theorem map_denote (sem : Semantics World L base context) :
    ∀ {sorts : List Ty} (arguments : Arguments (Term L) sorts)
      (world : World) (compatible : context world),
      (arguments.map sem.denote).map (fun atom => atom world compatible) =
        sem.denoteArgs arguments world compatible
  | [], .nil, _, _ => rfl
  | _ :: _, .cons _ tail, world, compatible => by
      simp only [Arguments.map, Semantics.denoteArgs, map_denote sem tail world compatible]

/-- A syntactic operation point maps to its semantic arguments followed by its result. -/
public theorem map_denote_graphPoint (sem : Semantics World L base context)
    {operation : L.Op} (arguments : Arguments (Term L) (L.params operation))
    (result : Term L (L.opResult operation)) (world : World) (compatible : context world) :
    ((arguments.map sem.denote).append (.cons (sem.denote result) .nil)).map
        (fun atom => atom world compatible) =
      (sem.denoteArgs arguments world compatible).append
        (.cons (sem.denote result world compatible) .nil) := by
  rw [Arguments.map_append, map_denote]
  rfl
...
@[expose] public def fieldTuple (sem : Semantics World L base context) (C : L.Constructor)
  ...
  relation := .inr (.field ⟨C, position, sort⟩)
  arguments := .cons owner (.cons child .nil)

/-- The graph tuple for a named operation application and a proposed result. -/
@[expose] public def graphTuple (sem : Semantics World L base context) (operation : L.Op)
  ...
  relation := .inr (.graph operation)
  arguments := (arguments.map sem.denote).append (.cons (sem.denote result) .nil)
...
/-- Decoding an equation headed by an owned operation gives exactly that operation's graph. -/
public theorem graphTuple_holds_iff (sem : Semantics World L base context)
    {operation : L.Op} {arguments : Arguments (Term L) (L.params operation)}
    {result : Term L (L.opResult operation)} {world : World} {compatible : context world} :
    (sem.graphTuple operation arguments result).Holds world compatible ↔
      sem.denote (.op operation arguments) world compatible =
        sem.denote result world compatible := by
  change sem.graphHolds operation
      (((arguments.map sem.denote).append (.cons (sem.denote result) .nil)).map
        (fun atom => atom world compatible)) ↔ _
  rw [map_denote_graphPoint]
  simp only [graphHolds, Arguments.init_append_singleton,
    Arguments.last_append_singleton, denote]

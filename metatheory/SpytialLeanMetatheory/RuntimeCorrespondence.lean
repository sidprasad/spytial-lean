...
These theorems connect the traced implementation to both public production
entry points. They establish exact erasure facts and deliberately make no
claim that Lean's evaluator or kernel has been verified here.
-/
...
/-- The public computed-value entry point is the evidence-producing walk
    followed by evidence erasure. -/

...
The surface is generated from spytial-core's language manifest
(`SpytialLean/SpecGenerated.lean`, regenerated by `just gen-spec`), so it
tracks the exact spytial-core release the widget embeds. Constructor doc
comments carry the authoritative field-by-field documentation; the tables
...
| `.size (width) (height)` | Set node dimensions (optionally per-selector) |

`orientation`, `cyclic`, `align`, and `group` also take `(hold := .never)` to
negate the constraint — assert the relationship must *not* hold.

### Visual directives

| Operation | Description |
|-----------|-------------|
| `.atomStyle (selector)` | Style nodes: `fillStyle`, `borderStyle`, `iconStyle`, `textStyle` blocks + `showLabel` |
| `.edgeStyle (field)` | Style a relation's edges: `lineStyle`/`textStyle` blocks, `showLabel`, `hidden` |
...
| `.flag (name)` | Set a whole-diagram flag (`.hideDisconnected`, `.hideDisconnectedBuiltIns`) |

Style blocks are structures with optional fields, e.g.
`.atomStyle (selector := "Tree") (borderStyle := { color := "#0066ff" })`.

`.atomColor`, `.edgeColor`, and `.icon` still work but are deprecated upstream
(superseded by `atomStyle`/`edgeStyle`); using them warns at elaboration time
with the rewrite to apply.
...
`TextSize`: `.small`, `.normal`, `.large` · `IconPlacement`: `.full`, `.badge` · `GroupEdge`: `.none`, `.toGroup`, `.fromGroup`

Because these are closed inductives — and numeric bounds and orientation's
direction rules are checked during elaboration — a typo or contradictory spec
fails at the authoring site instead of being silently ignored by the layout
engine.
...
2. **Spec** (`SpytialLean/SpecGenerated.lean` + `SpytialLean/Spec.lean`) — Typed Lean operations serialize to YAML for spytial-core's layout engine. The op type and serialization are generated from spytial-core's language manifest.

...
Unlike the `sum[e]` aggregator (broken → ∅, warned above), SGQ evaluates the
quantifier form correctly (probed against the installed 2.8.2 bundle), so it
carries no warning. Lowering parenthesizes it — SGQ extends the body maximally
right, so `(sum …) > 2` needs the parens the surface omits. -/

...
picks out pairs, testing every one; and `σ → List τ` computes the second column
instead of enumerating it — the shape to reach for on anything large.
-/
...
/-- An arity-2 selector: each node paired with the children it leans away from.
    Returning the values costs one call per node; the same relation written as
    `fun p c : RBNode => …` would test every *pair*. -/
...
spytial_spec RBNode [
  ...
  inferredEdge heavy lean (RBNode.heavySide) (lineStyle "red" dashed)
]

...
/-- A hole renders as a leaf but keeps its structural atom type, so a
    ...
    decomposer must never be handed a bare hole of its target type. A variable
    with an entry in `cfg.refinements` is not opaque: it draws the shape IYKYK
    established. -/
...
/-- Compiling such a constant yields its type's default instead of failing, so
    ...
    zero. `@[irreducible]` is not this: it has a body. Public: a view that
    recovers its value by evaluation needs the same guard. -/
...
private meta def emitNode (cfg : WalkConfig) (recurse : Expr → StateT WalkState MetaM String)
  ...
  match e with
  | .lit (.natVal n) =>
    modify fun s => s.addAtom { id := atomId, type := "Nat", label := dLabel?.getD (toString n) }

  | .lit (.strVal str) =>
    modify fun s =>
      s.addAtom { id := atomId, type := "String", label := dLabel?.getD s!"\"{str}\"" }

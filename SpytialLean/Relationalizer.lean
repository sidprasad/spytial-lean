...
/-- Session-local cache of *compiled* relationalizer functions keyed by def name
    together with a hash of the def's body, so a registered def is compiled at
    most once per process yet an interactive body edit (same name, new body)
    recompiles rather than serving the stale closure. The persistent extension is
...
public meta unsafe def getSpytialRelationalizerImpl (typeName : Name) :
  ...
  -- Key on the def's body hash, not just its name: an opaque def (`value? = none`)
  -- falls back to hash 0 (name-only behavior).
...
public meta partial def walkExpr (cfg : WalkConfig := {}) (eOrig : Expr) : StateT WalkState MetaM String := do
  ...
  if let .const typeConstName _ := tyWhnfForLookup.getAppFn then
    if let some relFn ← getSpytialRelationalizer? typeConstName then
      ...
      let id ← relFn eOrig (walkExpr cfg)
      modify (·.markSeen hash id)
      return id

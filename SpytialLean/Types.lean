...
/-- Why the production relationalizer emitted one tuple. The `terms` arrays
    ...
    until it supplies a separate certificate. -/
public inductive TupleOrigin where
  /-- Constructor fields and structure projections. -/
  | structural (terms : Array Expr)
  /-- A named application retained symbolically as a function-graph point. -/
  | symbolic (application : Expr) (terms : Array Expr)
  /-- A proposition/proof pair obtained from IYKYK knowledge. -/
  | proved (proposition proof : Expr) (terms : Array Expr)
  /-- A function-graph point obtained from a proof or definitional reduction. -/
  | observed (application result : Expr) (proof : Option Expr) (terms : Array Expr)
  /-- A point obtained by evaluating a function over a finite enumerable domain. -/
  | tabulated (application : Expr) (terms : Array Expr)
  /-- A representation edge, such as a stuck match's `scrutinee` relation. -/
  | synthetic (source : Expr) (terms : Array Expr)
...
public def TupleOrigin.terms? : TupleOrigin → Option (Array Expr)
  ...
  | .symbolic _ terms
  | .proved _ _ terms
  | .observed _ _ _ terms
  | .tabulated _ terms
  | .synthetic _ terms => some terms
  | .custom => none

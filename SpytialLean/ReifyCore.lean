...
@[expose] public def ofData (data : JsonDataInstance) : Except ReifyError ReifyDatum :=
...
@[expose] public def atom (datum : ReifyDatum) (id : String) : Except ReifyError JsonAtom :=
...
@[expose] public def child (datum : ReifyDatum) (owner field : String) :
...
public def decodeChildWith (datum : ReifyDatum) (owner field : String)
...
@[expose] public def childRepresentsWith (datum : ReifyDatum) (owner field : String)
...
public theorem decodeChildWith_complete
    {datum : ReifyDatum} {owner field : String}
...
@[expose] public def expectAtom (datum : ReifyDatum) (id expectedType : String) :
...
@[expose] public def constructorRepresents (datum : ReifyDatum) (root expectedType label : String)
    (fieldsRepresent : Bool) : Bool :=
...
public class Tier1Reification (α : Type u) [SpytialReify α] where
  representsAt : ReifyDatum → String → Nat → α → Bool
...
public theorem decodeAt_complete { α : Type u } [SpytialReify α] [Tier1Reification α]
    {datum : ReifyDatum} {root : String} {fuel : Nat} {value : α}
...
datum. Concrete equalities such as `reify (relationalize% x) = .ok x` are then ordinary kernel
theorems. `ReifyTest` contains such theorems for each Tier 1 shape and broader generated checks.

Call `reifyRooted` only when an explicit root is available. -/
...
@[expose] public def tier1Represents { α : Type u }
    ...
    (data : JsonDataInstance) (value : α) : Bool :=
  match ReifyDatum.ofData data with
  | .ok datum =>
      Tier1Reification.representsAt datum datum.root (datum.data.atoms.size + 1) value
  | .error _ => false
...
public abbrev Tier1Represents { α : Type u } [SpytialReify α] [Tier1Reification α]
    (data : JsonDataInstance) (value : α) : Prop :=
  tier1Represents data value = true
...
public theorem reify_of_tier1Represents
    ...
    {data : JsonDataInstance} {value : α} (h : Tier1Represents data value) :
    reify data = Except.ok value := by
  unfold Tier1Represents tier1Represents at h
  unfold reify
  cases hd : ReifyDatum.ofData data with
...
public def reifyRepr { α : Type u } [SpytialReify α] [Repr α]
    (data : JsonDataInstance) : Except ReifyError String :=
  (reify (α := α) data).map reprStr
...
public theorem reifyRepr_of_tier1Represents
    ...
    {data : JsonDataInstance} {value : α} (h : Tier1Represents data value) :
    reifyRepr (α := α) data = Except.ok (reprStr value) := by
...
public def reifyNatAt (datum : ReifyDatum) (root : String) (fuel : Nat) :
...
@[expose] public def natRepresentsAt
    (datum : ReifyDatum) (root : String) (fuel : Nat) (value : Nat) : Bool :=
  match fuel with
  | 0 => false
  | _ + 1 =>
      match datum.expectAtom root "Nat" with
...
public theorem reifyNatAt_complete {datum : ReifyDatum} {root : String} {fuel value : Nat}
...
public def reifyStringAt (datum : ReifyDatum) (root : String) (fuel : Nat) :
...
@[expose] public def stringRepresentsAt
    (datum : ReifyDatum) (root : String) (fuel : Nat) (value : String) : Bool :=
  match fuel with
  | 0 => false
  | _ + 1 =>
      match datum.expectAtom root "String" with
...
public theorem reifyStringAt_complete
    {datum : ReifyDatum} {root : String} {fuel : Nat} {value : String}
